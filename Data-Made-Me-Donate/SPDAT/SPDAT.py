...
mk_spdat_df.columns = mk_spdat_df.iloc[0]
mk_spdat_df = mk_spdat_df.drop(0)
...
for index, row in mk_spdat_df.iterrows():
    if not math.isnan(row['Participant Site Identifier']):
        ...
        if math.isnan(row['Score Change']):
            mk_spdat[key] = {}
            mk_spdat[key]['Beginning Score'] = int(float(row['Total Score']))
            mk_spdat[key]['New'] = 1
        else:
            mk_spdat[key]['Ending Score'] = int(float(row['Total Score']))
            mk_spdat[key]['Score Change'] = int(float(row['Score Change']))
            mk_spdat[key]['New'] = 0
...
plt.title('Clusters of mental health patients')
